...
class YamlFormMessage extends RenderElement {
  ...
  public function getInfo() {
    ...
    return [
      ...
      '#status_headings' => [],
      '#pre_render' => [
        [$class, 'preRenderYamlFormMessage'],
      ],
      '#theme_wrappers' => ['yamlform_message'],
    ];
  }
  ...
  public static function preRenderYamlFormMessage($element) {
    ...
    if ($message_close) {
      ...
      $close_attributes = [
        'aria-label' => t('close'),
        'class' => ['js-yamlform-message__link', 'yamlform-message__link'],
      ];
  ...
  public static function isClosed($storage, $id) {
    ...
    switch ($storage) {
      ...
      case self::STORAGE_USER:
        ...
        $values = $user_data->get('yamlform', $account->id(), $namespace) ?: [];
        return (isset($values[$id])) ? TRUE : FALSE;
  ...
  public static function setClosed($storage, $id) {
    ...
    switch ($storage) {
      ...
      case self::STORAGE_USER:
        ...
        $values = $user_data->get('yamlform', $account->id(), $namespace) ?: [];
        $values[$id] = TRUE;
        $user_data->set('yamlform', $account->id(), $namespace, $values);
    }
  }
